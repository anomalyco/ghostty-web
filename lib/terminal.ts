...
export class Terminal implements ITerminalCore {
  ...
  private writeInternal(data: string | Uint8Array, callback?: () => void): void {
    ...
    this.processTerminalResponses();
    this.processTerminalQueries(data);
  ...
  private processTerminalQueries(data: string | Uint8Array): void {
    const input = this.terminalQueryBuffer + (typeof data === 'string' ? data : new TextDecoder().decode(data));
    const query = /(?:(?:\x1b\]|\x9d)(10|11);\?(?:\x07|\x1b\\|\x9c))|(?:(?:\x1b\[|\x9b)\?2031\$p)/g;
    let consumed = 0;
    ...
    while ((match = query.exec(input)) !== null) {
      ...
      if (!match[1]) {
        // Browser PTYs can replay this probe after the client's capability timeout.
    ...
    const start = Math.max(
      tail.lastIndexOf('\x1b]'),
      tail.lastIndexOf('\x9d'),
      tail.lastIndexOf('\x1b['),
      tail.lastIndexOf('\x9b')
    );
